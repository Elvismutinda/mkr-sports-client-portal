...
export default function AppNavBar({ toggleMenu }: AppNavBarProps) {
  ...
  // Replace with your actual notifications hook
        ...
        <Popover open={isPopoverOpen} onOpenChange={setPopoverOpen}>
          <PopoverTrigger asChild>
            <button className="relative cursor-pointer">
              <Bell className="h-6 w-6 text-gray-400 hover:text-mkr-yellow transition-colors" />
              {notificationCount > 0 && (
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="outline-none">
              <Avatar className="h-9 w-9 cursor-pointer ring-2 ring-transparent hover:ring-mkr-yellow transition-all">
                <AvatarFallback className="bg-mkr-slate text-mkr-yellow font-bold text-sm uppercase">
